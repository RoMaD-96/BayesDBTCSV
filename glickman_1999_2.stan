...
parameters {
    ...
    real log_sigma2_init;  // Log of variance parameters
    real log_sigma2;  // Log of variance parameters
...
transformed parameters {
    ...
    real<lower=0> sigma_init = sqrt(exp(log_sigma2_init));          // sd parameters (σ)
    real<lower=0> sigma = sqrt(exp(log_sigma2));          // sd parameters (σ)
...
model {
    ...
    target += normal_lpdf(log_sigma2_init | 0, omega);
    target += normal_lpdf(log_sigma2| 0, tau);

    // AR(1) process for strength parameters using stochastic variances
    // This implements Glickman's equation (2): c^(t+1) ~ N(c^(t), σ²^(t+1))
    for(k in 1:nteams){
        target+=normal_lpdf(logStrength_raw[1, k] | mean_logStrength, sigma_init);
    }

    for (t in 2:ntimes_rank) {
        for (k in 1:nteams) {
            target += normal_lpdf(logStrength_raw[t, k] | logStrength_raw[t-1, k], sigma);
        }
    }
